...
ICP_ConnectOdometry::ICP_ConnectOdometry(std::vector<Eigen::Vector3f> & tvecs_,
 ...
 : tvecs_(tvecs_),
   ...
   nsmaps_g_prev_(nsmaps_g_prev_),
   nsmaps_curr_(nsmaps_curr_),
   intr(intr),
...
ICP_ConnectOdometry::~ICP_ConnectOdometry()
{

}

void ICP_ConnectOdometry::reset()
{
    return;
}

CloudSlice::Odometry ICP_ConnectOdometry::getIncrementalTransformation(Eigen::Vector3f & trans,
                                                               ...
                                                               unsigned short * depthData)
{
    ...
    for (int level_index = ICP_ConnectOdometry::LEVELS - 1; level_index >= 0; --level_index)
    {
        ...
        DeviceArray2D<float> & nsmap_g_prev = nsmaps_g_prev_[level_index];
        DeviceArray2D<float> & nsmap_curr = nsmaps_curr_[level_index];
        ...
        for(int iter = 0; iter < iter_num; ++iter)
        {
            ...
            float residual[2];
 //           std::cout << "-----test-----" << "icp_step_connect" << std::endl;
//            icpStep(device_Rcurr,
            icpStep_Connect(device_Rcurr,
                    device_tcurr,
                    vmap_curr,
                    nmap_curr,
                    nsmap_curr,
...
Eigen::MatrixXd ICP_ConnectOdometry::getCovariance()
{
